...
export default function BubbleSort() {
  ...
  const draw = (p5) => {
    ...
    if (state.begin) {
        ...
        if (state.time % 5 === 0) {
            state.time = 0;
            if (state.minIndex < state.n && !state.completed) {
                if (state.placer == state.n) {
                    if (!state.swapped) {
                ...
                if (state.lines[state.placer] > state.lines[state.placer + 1]) {
                    [state.lines[state.placer], state.lines[state.placer + 1]] = [
                        state.lines[state.placer + 1],
                        state.lines[state.placer]
                    ];
                    state.colors[state.placer] = 1;
                    state.colors[state.placer + 1] = 1;
                    state.swapped = true;
                }
                if (!state.completed) {
                    state.placer += 1;
